...
void readline(char* buffer, int maxlen) {
    ...
    while (1) {
        ...
        unsigned char key_pressed = !(scancode & 0x80);
        unsigned char base_code = scancode & 0x7F;
        ...
        if (key_pressed) {
            ...
            if (base_code == 0x1C) {
                ...
                print("\n");
                /* Wait for release */
                while ((inb(0x60) & 0x7F) != 0x1C || !(inb(0x60) & 0x80)) {
                    /* wait */
                }
                return;
            }
            ...
            if (pos < maxlen - 1) {
                ...
                while ((inb(0x60) & 0x7F) != base_code || !(inb(0x60) & 0x80)) {
                    /* wait until release */
                }
            }
        }
